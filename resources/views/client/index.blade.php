  ...
  <div class="col-lg-8">
    <div class="card mb-3">
      <div class="card-body">
        ...
        <a href="{{ route('client.export.pdf') }}" class="btn btn-sm btn-outline-light mb-2">
          Download PDF
        </a>
        ...
        <div class="table-responsive">
          <table class="table table-dark table-hover align-middle">
            ...
            <tbody>
                ...
                <tr>
                  ...
                  <td>
                    @if(!empty($task->due_date))
                      {{ optional($task->due_date)->format('Y-m-d') ?? $task->due_date }}
                    @else
                      —
                    @endif
                  </td>
                  ...
                  <td>
                    {{-- if your schema stores a name string, fallback to relation --}}
                    {{ $task->assigned_to ?? ($task->assignedStaff->name ?? '—') }}
                  </td>

                  {{-- NEW: read-only progress for clients --}}
                  <td style="min-width:200px;">
                    ...
                    {{-- Optional: show proof attachments if present --}}
                    @if(isset($task->attachments) && $task->attachments->count())
                      <div class="mt-1 small">
                        @foreach($task->attachments as $att)
                          <a href="{{ \Illuminate\Support\Facades\Storage::url($att->path) }}" target="_blank">
                            {{ $att->original_name }}
              ...
              @empty
                <tr>
                  <td colspan="7" class="text-center text-muted">No tasks yet.</td> {{-- updated colspan --}}
                </tr>
              @endforelse
            </tbody>
          </table>
